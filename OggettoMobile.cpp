...
void OggettoMobile::sposta(float dx, float dy, int larghezza, int altezza)
{
    ...
    // Se si miove salva la direzione per calcolare l'angolo
    if (dx != 0.0f || dy != 0.0f) {
        this->spostX = dx;
        this->spostY = dy;
    }
...
void OggettoMobile::aggiungiOggettoMobile(InterfacciaUtente& ui)
{
    ...
    {
        // Scala il PNG affinché il diametro coincida con raggio x 2
        float scala = 0;
    ...
    {
        // Modalità testo: disegna un cerchio pieno
        ui.aggiungiCerchioRigCol(Posizione(this->centroX, this->centroY),
            this->raggio, this->coloreForma);
    }
}